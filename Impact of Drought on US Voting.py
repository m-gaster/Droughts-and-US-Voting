...
continuous_agr_indep_vars = 'drought_score*agricultural_gdp + DPres + state + year'
...
collapsed_elec_df.plot.scatter(x='drought_score', y='totalturnout', c='agricultural', colormap='jet', figsize=(15,10), xlabel='drought score', ylabel = 'Total Turnout').autoscale()
...
incumbent_voteshare_change_regression = smf.ols(formula='share_diff ~ drought_score*agricultural*DPres + state + year', data=elec_df)
...
collapsed_elec_df.rename(columns={'Republican_predicted_voteshare_all_years': 'Republican_pred_voteshare_minus_actual_all_years'})


# In[ ]:


collapsed_elec_df['Republican_pred_voteshare_minus_actual_all_years'].min()
...
#     * ~~Right now, they don't end up with any drought info in elec_df by the end  ~~
# * Figure out clustered standard errors
# * Collapse elec_df using groupby
#     * * elec_df.groupby(['state', 'year', 'district']) should work?
#     
# * incumbents are currently all who ran last time, not whoever won
